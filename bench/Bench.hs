...
golds = take 5 $ unsafePerformIO $ do
    ...
    bins <- sequence (fmap (fmap BU.unpack . B.readFile . ("test/testcase-output/"++)) files)
...
main = do
    ...
    defaultMain [
        ...
        --bgroup "Rendering" $ fmap (\(x, y) -> bench ("TestCase " ++ x) $ nfIO (phantomjs ("test/testcase-output/" ++ x))) golds
        ]
